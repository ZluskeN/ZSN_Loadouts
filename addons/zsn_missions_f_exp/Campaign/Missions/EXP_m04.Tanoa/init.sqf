...
private _introVideosSubtitles = ["", "a3\Missions_F_Exp\Campaign\Briefings\EXP_m04_briefing.sqf"];
private _introVideos = if (!(BIS_devMode)) then {["a3\missions_f_exp\video\EXP_m04_vTitlecard.ogv", "a3\missions_f_exp\video\exp_m04_vin.ogv"]} else {[]};
...
[
	...
	EVENT_STARTED,
	{
		if (isServer) then {
			...
			BIS_started = true;
			publicVariable "BIS_started";

			// Enable respawn
			BIS_respawnEnabled = true;

			// Execute intro
			if (isDedicated) then {[] spawn BIS_fnc_EXP_m04_intro};
			[[], "BIS_fnc_EXP_m04_intro", ([] call BIS_fnc_listPlayers)] call BIS_fnc_MP;
		};
	}
...
if (!(isDedicated) && { !(BIS_devMode) }) then {
	...
	[] spawn {
		...
		[player, "BIS_fnc_EXP_m04_addRespawn", false] call BIS_fnc_MP;
...
{if (!(isNil _x)) then {(missionNamespace getVariable [_x, objNull]) setPitch 1}} forEach ["BIS_HQ", "BIS_SOF"];
