scriptName "CSAT_Medic";

if (!hasInterface) exitWith { ["Loadout running on dedicated server - %1", "CSAT_Medic"] call BIS_fnc_logFormat; };
waitUntil { time > 0 };
waitUntil { !isNull player };
if (!local _this) exitWith { ["Loadout where unit is not local - %1 / %2", "CSAT_Medic", _this] call BIS_fnc_logFormat; };
