if (isServer) then
{
	...
	BIS_fnc_onLaptopDestruction =
	{
		...
		["On laptop destruction %1", _laptop] call BIS_fnc_logFormat;
	};
	...
	if (!isNull _laptop) then
	{
		...
		{
			["Explosion event handler fired for laptop %1", _this select 0] call BIS_fnc_logFormat;
			...
			if (isNil { (_this select 0) getVariable "BIS_hvt_intelDestroyed" } && !isNil { (_this select 0) getVariable "BIS_hvt_intelDownloadedBy" }) then
			{
				...
				["Explosion event handler destroyed laptop %1", _this select 0] call BIS_fnc_logFormat;
			};
		}]];
		...
		{
			["Hit event handler fired for laptop %1", _this select 0] call BIS_fnc_logFormat;
			...
			if (isPlayer (_this select 1) && isNil { (_this select 0) getVariable "BIS_hvt_intelDestroyed" } && !isNil { (_this select 0) getVariable "BIS_hvt_intelDownloadedBy" }) then
			{
				...
				[_this select 0] call BIS_fnc_onLaptopDestruction;
				["Hit event handler destroyed laptop %1", _this select 0] call BIS_fnc_logFormat;
			};
		}]];

		// Log
		["Laptop.sqf: Events added to %1", _laptop] call BIS_fnc_logFormat;
	};
};